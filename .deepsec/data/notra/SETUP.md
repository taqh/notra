# Agent setup for `notra`

This is a deepsec scanning workspace. Project `notra` was just registered
(target: `..`). Setup is incomplete — `data/notra/INFO.md`
...
2. **Fill in `data/notra/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id notra
pnpm deepsec process --project-id notra
```
